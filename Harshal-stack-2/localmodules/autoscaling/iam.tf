...
resource "aws_iam_role" "be_inst_role" {
  name               = "harshal-${var.env_name}-s3-cw-rds-ssm-access"
  assume_role_policy = data.aws_iam_policy_document.ec2_assume_role_policy.json
}

resource "aws_iam_role" "fe_inst_role" {
  name               = "harshal-${var.env_name}-s3-cw-rds-ssm-access-fe"
  assume_role_policy = data.aws_iam_policy_document.ec2_assume_role_policy.json
}

resource "aws_iam_policy_attachment" "s3_full_access" {
  name       = "harshal-${var.env_name}-s3-access-policy"
